...
import { verticalBarOptions, stackedBarOptions, barOptions, scatterBubblePlotOptions, LineChartOptions, groupdBarOptions, GRAPH_TYPES } from './utils';


export default function Graph({ labels, datasets, type, height, width }: any) {
    const data = {
        labels,
        datasets
    };
    const RenderedChart = () => {
        switch (type) {
            ...
            case GRAPH_TYPES.GROUPED_BAR_CHART:
                return <Bar options={groupdBarOptions as any} data={data} />
            case GRAPH_TYPES.LINE_CHART:
                return <Line options={LineChartOptions as any} data={data} />
